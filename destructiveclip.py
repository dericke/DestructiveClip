...
class DestructiveClip(inkex.Effect):
    ...
    def simplepathToLineSegments(self, path):
        ...
        for cmd in path:
            ...
            if cmd[0] == "M":  # moveto
                ...
                errors.add(
                    "Curve node detected (svg type C), this node will be handled as a regular node"
                )
            else:
                errors.add(
                    "Invalid node type detected: {}. This script only handle type M, L, Z".format(
                        cmd[0]
                    )
                )
            prev = this
    ...
    def linesgmentsToSimplePath(self, lineSegments):
        ...
        for line in lineSegments:
            ...
            if end is None:
                ...
                self.approxEqual(end[0], start[0])
                and self.approxEqual(end[1], start[1])
            ):
                path.append(
                    ["M", start]
                )  # only move if previous end not within tolerance of this start
            end = line[1]
            path.append(["L", end])
        return path
    ...
    def lineIntersection(self, L1From, L1To, L2From, L2To):
        ...
        if not self.approxEqual(denominator, 0.0):
            s = (
                -dL1[1] * (L1From[0] - L2From[0]) + dL1[0] * (L1From[1] - L2From[1])
            ) / denominator
            t = (
                +dL2[0] * (L1From[1] - L2From[1]) - dL2[1] * (L1From[0] - L2From[0])
            ) / denominator
    ...
    def insideRegion(self, point, lineSegments, lineSegmentsMaxX):
        ...
        ray = [
            point,
            [lineSegmentsMaxX * 2.0, point[1]],
        ]  # hz line to right of point, extending well outside MBR
        crossings = 0
        for line in lineSegments:
    ...
    def cullSegmentedLine(self, segmentedLine, lineSegments, lineSegmentsMaxX):
        ...
        for segment in segmentedLine:
            if self.insideRegion(
                self.midPoint(segment), lineSegments, lineSegmentsMaxX
            ):
    ...
    def clipLine(self, line, lineSegments):
        ...
        for segment in lineSegments:
            ...
            for line in linesRead:
                intersect = self.lineIntersection(
                    line[0], line[1], segment[0], segment[1]
                )
    ...
    def clipLineSegments(self, lineSegmentsToClip, clippingLineSegments):
        ...
        for lineToClip in lineSegmentsToClip:
            clippedLines.extend(
                self.cullSegmentedLine(
                    self.clipLine(lineToClip, clippingLineSegments),
                    clippingLineSegments,
                    self.maxX(clippingLineSegments),
                )
            )
        return clippedLines
        ...
        for id in self.options.ids:  # the selection, top-down
            ...
            if node.tag == pathTag:
                if clippingLineSegments is None:  # first path is the clipper
                    (clippingLineSegments, errors) = self.simplepathToLineSegments(
                        node.path.to_arrays()
                    )
                    ...
                    if len(clippedSegments) != 0:
                        path = str(
                            inkex.Path(self.linesgmentsToSimplePath(clippedSegments))
                        )
                        node.set("d", path)
                    else:
                        # don't put back an empty path(?)  could perhaps put move, move?
                        inkex.errormsg(
                            "Object {} clipped to nothing, will not be updated.".format(
                                node.get("id")
                            )
                        )
            elif node.tag == groupTag:  # we don't look inside groups for paths
                inkex.errormsg(
                    "Group object {} will be ignored. Please ungroup before running the script.".format(
                        id
                    )
                )
            else:  # something else
                inkex.errormsg(
                    'Object {} is not of type path ({}), and will be ignored. Current type "{}".'.format(
                        id, pathTag, node.tag
                    )
                )
